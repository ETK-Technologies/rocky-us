...
export async function GET(request) {
  ...
  try {
    ...
    const contentTypeIds = {
      post: searchResponse.data.filter(item => item.subtype === 'post').map(item => item.id),
      page: searchResponse.data.filter(item => item.subtype === 'page').map(item => item.id),
      product: searchResponse.data.filter(item => item.subtype === 'product').map(item => item.id)
    };
    ...
    if (contentTypeIds.post.length > 0) {
      fetchPromises.push(
        axios.get(`${BASE_URL}/wp-json/wp/v2/posts`, {
          params: {
            ...
            if (resultsMap.has(post.id)) {
              ...
              if (post._embedded?.['wp:featuredmedia']?.[0]) {
                ...
                if (featuredMedia.source_url) {
                  ...
                  imageUrl = sizes.medium?.source_url || sizes.full?.source_url;
                }
              }
    ...
    if (contentTypeIds.page.length > 0) {
      fetchPromises.push(
        axios.get(`${BASE_URL}/wp-json/wp/v2/pages`, {
          params: {
        ...
        .then(response => {
          if (response.data.length > 0) {
            console.log('Page data example:', JSON.stringify(response.data[0]).substring(0, 500));
          }
    ...
    if (contentTypeIds.product.length > 0) {
      fetchPromises.push(
        axios.get(`${BASE_URL}/wp-json/wc/v3/products`, {
          params: {
        ...
        .then(response => {
          if (response.data.length > 0) {
            console.log('Product data example:', JSON.stringify(response.data[0]).substring(0, 500));
          }
    ...
    const enhancedResults = searchResponse.data.map(item => resultsMap.get(item.id));
    console.log(`Enhanced ${enhancedResults.length} results, ${enhancedResults.filter(r => r.image).length} with images`);
