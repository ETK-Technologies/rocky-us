...
const QuizStepRenderer = ({
  ...
  // Handle photo upload step (step 14)
  if (stepIndex === 14) {
    return (
      <PhotoUploadStep
        onContinue={handleNext}
        onBack={() => handleAction("navigate", 13)}
        questionnaire="anti-aging"
        onPhotoUpload={async (frontPhotoUrl, sidePhotoUrl) => {
          // Store photo URLs in userData
          setAnswers((prev) => ({
            ...prev,
          ...
          // Defer submit to once-per-step by letting handleNext run after upload
        }}
      />
    );
  }

  // Handle ID upload step (step 15)
  if (stepIndex === 15) {
    return (
      <IdUploadStep
        onContinue={handleNext}
        onBack={() => handleAction("navigate", 14)}
        questionnaire="anti-aging"
  ...
  // Handle recommendation step (step 17)
  if (stepIndex === 17) {
      ...
      <div className="w-full md:w-[520px] mx-auto px-5 md:px-0 mt-6">
        <div className="text-center">
          ...
          <p className="text-gray-600 mb-8">
            Your personalized anti-aging treatment has been added to your cart.
