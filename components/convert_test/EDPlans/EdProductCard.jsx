...
const EdProductCard = ({ product }) => {
  ...
  // Handle dosage selection: add the selected product to cart client-side, then continue
    const {
      setSelectedProduct: setCtxSelectedProduct,
      setSelectedDosage: setCtxSelectedDosage,
    } = useQuiz();
  ...
  const handleDosageContinue = () => {
    ...
    // Build payload to persist for downstream pages
    try {
      const payload = {
        // original onboarding identifiers
        productIds: getProductIDs(),
        productName: product.name,
        // enrich with display data so checkout/payment pages can show full summary
        image: product.image,
        price: currentPrice,
      ...
      sessionStorage.setItem("ed_onboarding_payload", JSON.stringify(payload));

      // update context selection so modal/checkout can read it
      try {
        setCtxSelectedProduct({
        ...
        setCtxSelectedDosage(payload.dosage);
      } catch (e) {
        // ignore
      }
    } catch (e) {
    ...
    // Attempt to add to cart via client API. On success, navigate to Almost page.
    // try {
    ...
    //   router.push(
    //     `/almost?productIds=${encodeURIComponent(
    //       getProductIDs()
    ...
    // } catch (err) {
    //   logger.error("Error adding ED product to cart:", err);
    // }
    handleCheckout(); // Directly proceed to checkout for simplicity
  };
