...
export class PaymentService {
  ...
  /**
   * Process payment with Paysafe using Card Payments API for authorization-only
   * @param {Object} paymentData - Payment information
   * @returns {Promise<Object>} Payment response
   */
  async processPayment(paymentData) {
    ...
    // Prepare Paysafe payment request for authorization-only
    const paysafePayload = {
      merchantRefNum: `order_${order_id}_${Date.now()}`,
      amount: Math.round(amount * 100), // Convert to cents
      settleWithAuth: false, // Authorization only - no immediate settlement
      preAuth: true,
      currency: currency,
      card: {
        cardNum: cardNumber.replace(/\s/g, ""),
