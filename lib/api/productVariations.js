...
export const fetchProductVariations = async (productId, attributes = null) => {
  try {
    ...
    if (variationCache[productId]) {
      console.log(`Using cached variations for product ${productId}`);
      return variationCache[productId];
    }

    // Fetch variations from API
    console.log(`Fetching variations for product ${productId} from API`);
    ...
    console.error(`Error fetching product variations for ${productId}:`, error);
...
export const findVariationId = async (productId, attributes = {}) => {
  try {
    ...
    console.error(`Error finding variation ID for ${productId}:`, error);
    return null;
  }
};
